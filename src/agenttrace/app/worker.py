...
async def run_analysis_pipeline(job: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        repo_id = str(job["repository_id"]) if job.get("repository_id") else None
        ...
        if snap_id and repo_id:
            conn.execute(
                ...
                """,
                {"snap_id": snap_id, "repo_id": repo_id}
            )
    except Exception as insert_exc:
        logger.warning("Defensive insert of parent records failed in worker: %s", insert_exc)
    ...
    try:
        ...
        logger.debug("Could not fetch repository from db, fallback to default: %s", exc)
    ...
    try:
        ...
        logger.debug("Could not fetch snapshot from db, fallback to default: %s", exc)
    ...
    logger.info("Worker executing analysis graph pipeline for job_id=%s", run_id)
    ...
    logger.info("Worker job %s persisted successfully", run_id)
